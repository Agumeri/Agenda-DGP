...
export const createAdmin = async (req, res) => {
    ...
    const[rows] = await connection.query('SELECT COUNT(*) FROM administrador')

    const userId = user.insertId;
    const adminId = "admin" + (rows[0]["COUNT(*)"] + 1)
    
    const [result] = await connection.query("INSERT INTO administrador(id_admin, id_usuario_inAdmin) VALUES (?, ?)",[
        adminId,
        userId
    ])   
...
export const getAdminByID = async (req,res) => {
    ...
    const [admin] = await connection.query('SELECT * FROM administrador WHERE id_admin = ?',[id_admin])
    console.log(admin);
...
export const getAdmin = async (req,res) => {
    ...
    const [rows]  = await connection.query(
        'SELECT nombre_usuario, correo_electronico, permisos FROM usuario, administrador WHERE usuario.id = administrador.id_usuario_inAdmin'
        )
    res.json(rows)
}
...
export const updateAdmin = async (req,res) => {
    ...
    const id_admin = req.params.id
    const [admin] = await connection.query("SELECT id_usuario_inAdmin FROM administrador WHERE id_admin = ?",[id_admin]);
...
export const deleteAdmin = async (req,res) => {
    ...
    const [delete_admin_result] = await connection.query('DELETE FROM administrador WHERE id_admin = (?)',[id_admin]);
